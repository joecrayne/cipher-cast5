...
module Crypto.Cipher.Cast5 where

import Crypto.Cipher.Feistel
...
type Maskkey = Word32
type Rotkey  = Word32
type Subkey = (Maskkey,Rotkey)
type Func = Subkey -> Word32 -> Word32

type Key = (Word32,Word32,Word32,Word32)
data Cast5 keySize = Cast5 ![Subkey] ![Func] 
                           ![Subkey] ![Func] 
                           !Key

instance Eq (Cast5 size) where 
  Cast5 _ _ _ _ a == Cast5 _ _ _ _ b  =  a==b
...
data KeySize_120
data KeySize_128
type CAST5_40 = Cast5 KeySize_40
type CAST5_48 = Cast5 KeySize_48
type CAST5_56 = Cast5 KeySize_56
type CAST5_64 = Cast5 KeySize_64
type CAST5_72 = Cast5 KeySize_72
type CAST5_80 = Cast5 KeySize_80
type CAST5_88 = Cast5 KeySize_88
type CAST5_96 = Cast5 KeySize_96
type CAST5_104 = Cast5 KeySize_104
type CAST5_112 = Cast5 KeySize_112
type CAST5_120 = Cast5 KeySize_120
type CAST5_128 = Cast5 KeySize_128

class Cast5Bits size where cast5bits :: size -> Int
instance Cast5Bits KeySize_40 where cast5bits _ = 40
instance Cast5Bits KeySize_48 where cast5bits _ = 48
instance Cast5Bits KeySize_56 where cast5bits _ = 56
instance Cast5Bits KeySize_64 where cast5bits _ = 64
instance Cast5Bits KeySize_72 where cast5bits _ = 72
instance Cast5Bits KeySize_80 where cast5bits _ = 80
instance Cast5Bits KeySize_88 where cast5bits _ = 88
instance Cast5Bits KeySize_96 where cast5bits _ = 96
instance Cast5Bits KeySize_104 where cast5bits _ = 104
instance Cast5Bits KeySize_112 where cast5bits _ = 112
instance Cast5Bits KeySize_120 where cast5bits _ = 120
instance Cast5Bits KeySize_128 where cast5bits _ = 128

instance Cast5Bits size => Serialize (Cast5 size) where
    put (Cast5 _ _ _ _ (a,b,c,d)) = 
        putByteString . S.concat . map fromW32Pair $ [(a,b),(c,d)]
    get = do
        let len = cast5bits (undefined::size)
        bs <- getBytes (len `div` 8) 
        let Just key = buildKey bs
        return key

instance Cast5Bits size => BlockCipher (Cast5 size) where
    blockSize = Tagged 64
    encryptBlock (Cast5 subkeys fs _ _ key) = 
        fromW32Pair . coreCrypto nrounds subkeys fs . toW32Pair
      where 
        nrounds = numberOfRounds (cast5bits (undefined::size))
    decryptBlock (Cast5 _ _ subkeys fs key) = 
        fromW32Pair . coreCrypto nrounds subkeys fs . toW32Pair
      where 
        nrounds = numberOfRounds (cast5bits (undefined::size))
    buildKey bs = Just (Cast5 subs fs subs' fs' key)
      where
        nrounds = numberOfRounds (cast5bits (undefined::size))
        key = initKey bs
        fs    = cycle [f1,f2,f3]
        subs  = subkeys key
        fs'   = drop (3 - (nrounds `rem` 3)) $ cycle [f3,f2,f1]
        subs' = (reverse . take nrounds $ subs)
    keyLength = Tagged (cast5bits (undefined::size))
...
{-# INLINE (<<<) #-}
x <<< r = rotate x (fromIntegral r)

{-# INLINE only5bits #-}
only5bits lsw = lsw .&. 0x1F

initKey bs = (x0x1x2x3, x4x5x6x7, x8x9xAxB, xCxDxExF) 
 where
    len = S.length bs
    zeros = S.replicate (16-len) 0
    words bs | S.null bs = []
             | otherwise = w : words bs'
                where (w,bs') = S.splitAt 4 bs
    [x0x1x2x3, x4x5x6x7, x8x9xAxB, xCxDxExF]
        = map decode32be $ words (bs `S.append` zeros)


numberOfRounds bits | bits <= 80 = 12
                    | otherwise  = 16

coreCrypto round_count subkeys fs (l0,r0) = swap (lrs !! round_count)
 where
    lrs = (l0,r0):rounds fs subkeys l0 r0
    rounds (f:fs) (maskrot:subkeys) l0 r0 
        =(l1,r1):rounds fs subkeys l1 r1
      where 
            l1 = r0
            r1 = l0 ^ f maskrot r0 
    ...
    group2 _ _        = []
    (x:xs) !! 0 = x
    (x:xs) !! i = force x `seq` xs !! (i-1)
    force (a,b) = a `seq` b


f1 (kmi,kri) d = ((s1#ia ^ s2#ib) - s3#ic) + s4#id
 where
    i = ((kmi + d) <<< kri)
    (# ia,ib,ic,id #) = splitWord32 i

f2 (kmi,kri) d = ((s1#ia - s2#ib) + s3#ic) ^ s4#id
 where
    i = ((kmi ^ d) <<< kri)
    (# ia,ib,ic,id #) = splitWord32 i

f3 (kmi,kri) d = ((s1#ia + s2#ib) ^ s3#ic) - s4#id
 where
    i = ((kmi - d) <<< kri)
    (# ia,ib,ic,id #) = splitWord32 i


subkeys key = zip mask_keys rotate_keys
 where
    (mask_keys, map only5bits -> rotate_keys) = splitAt 16 . keys1 $ key


keys1 (x0x1x2x3, x4x5x6x7, x8x9xAxB, xCxDxExF) = 
    [k1,k2,k3,k4] ++ keys2 (z0z1z2z3, z4z5z6z7, z8z9zAzB, zCzDzEzF)
 where

    (# x0,x1,x2,x3 #) = splitWord32 x0x1x2x3
    (# x4,x5,x6,x7 #) = splitWord32 x4x5x6x7
    (# x8,x9,xA,xB #) = splitWord32 x8x9xAxB
    (# xC,xD,xE,xF #) = splitWord32 xCxDxExF

    (# z0,z1,z2,z3 #) = splitWord32 z0z1z2z3
    (# z4,z5,z6,z7 #) = splitWord32 z4z5z6z7
    (# z8,z9,zA,zB #) = splitWord32 z8z9zAzB
    (# zC,zD,zE,zF #) = splitWord32 zCzDzEzF
...
keys2 (z0z1z2z3, z4z5z6z7, z8z9zAzB, zCzDzEzF) = 
    [k5,k6,k7,k8] ++ keys3 (x0x1x2x3, x4x5x6x7, x8x9xAxB, xCxDxExF)
  where
    (# z0,z1,z2,z3 #) = splitWord32 z0z1z2z3
    (# z4,z5,z6,z7 #) = splitWord32 z4z5z6z7
    (# z8,z9,zA,zB #) = splitWord32 z8z9zAzB
...
keys3 (x0x1x2x3, x4x5x6x7, x8x9xAxB, xCxDxExF) = 
    [k9,k10,k11,k12] ++ keys4 (z0z1z2z3, z4z5z6z7, z8z9zAzB, zCzDzEzF)
  where
    (# z0,z1,z2,z3 #) = splitWord32 z0z1z2z3
    (# z4,z5,z6,z7 #) = splitWord32 z4z5z6z7
    (# z8,z9,zA,zB #) = splitWord32 z8z9zAzB
...
keys4 (z0z1z2z3, z4z5z6z7, z8z9zAzB, zCzDzEzF) =
    [k13,k14,k15,k16] ++ keys1 (x0x1x2x3, x4x5x6x7, x8x9xAxB, xCxDxExF)
  where
    (# z0,z1,z2,z3 #) = splitWord32 z0z1z2z3
    (# z4,z5,z6,z7 #) = splitWord32 z4z5z6z7
    (# z8,z9,zA,zB #) = splitWord32 z8z9zAzB
...
hasSize :: Cast5 size -> size -> a
hasSize _ _ = undefined


data StaticTest size = StaticTest {
        keysize :: size,
        keybytes :: S.ByteString,
        plaintext :: S.ByteString,
        ciphertext :: S.ByteString,
        sched :: Maybe [Subkey]
    }

runTest test = do
    let Just key = buildKey (keybytes test)
        _ = hasSize key (keysize test)
        computed_sched  = subkeys key' where Cast5 _ _ _ _ key' = key
    case sched test of
        Nothing    -> return ()
        Just sched | sched == computed_sched
                        -> putStrLn "subkeys passed."
                   | otherwise
                        -> putStrLn $
                            unlines 
                                  [ "subkeys failed"
                                  , "expected = "++show sched
                                  , "computed = "++show computed_sched ]
    let computed_ciphertext = encryptBlock key (plaintext test)
    if computed_ciphertext == ciphertext test
        then putStrLn "encrypt passed."
        else putStrLn $ unlines [ "encrypt failed."
                                , "expected = "++show (ciphertext test)
                                , "computed = "++show computed_ciphertext ]
    let computed_plaintext = decryptBlock key (ciphertext test)
        decrypt_passed = computed_plaintext == plaintext test
    if decrypt_passed
        then putStrLn "decrypt passed."
        else putStrLn $ unlines [ "decrypt failed."
                                , "expected = "++show (plaintext test)
                                , "computed = "++show computed_plaintext]
    let when cond body = if cond then body else return ()
    when (not decrypt_passed) $ do
        let computed_plaintext = decryptBlock key computed_ciphertext
        if computed_plaintext == plaintext test
            then putStrLn "inverse passed."
            else putStrLn $ unlines [ "inverse failed."
                                    , "expected = "++show (plaintext test)
                                    , "computed = "++show computed_plaintext]
...
main = do
    let keys128 = undefined :: KeySize_128
        keys80  = undefined :: KeySize_80
        keys40  = undefined :: KeySize_40
    putStrLn "Test 128-bit"
    runTest $ StaticTest {
                keysize = keys128,
                keybytes = [bytes| 01 23 45 67 12 34 56 78 23 45 67 89 34 56 78 9A |],
                plaintext  = [bytes| 01 23 45 67 89 AB CD EF |],
                ciphertext = [bytes| 23 8B 4F E5 84 7E 44 B2 |],
                sched = Just [ (0xbc173e26,21)
                             , (0x78a207ef,27)
                             , (0xece0a7f5,1)
                             , (0x7cb0fb6b,5)
                             , (0xa5d2d636,3)
    ...
    putStrLn "Test 80-bit"
    runTest $ StaticTest {
                keysize = keys80,
                keybytes   = [bytes| 01 23 45 67 12 34 56 78 23 45 |],
                plaintext  = [bytes| 01 23 45 67 89 AB CD EF |],
                ciphertext = [bytes| EB 6A 71 1A 2C 02 27 1B |],
                sched = Nothing
                }
    putStrLn "Test 40-bit"
    runTest $ StaticTest {
                keysize = keys40,
                keybytes   = [bytes| 01 23 45 67 12 |],
                plaintext  = [bytes| 01 23 45 67 89 AB CD EF |],
                ciphertext = [bytes| 7A C8 16 D1 6E 9B 30 2E |],
                sched = Nothing
                }

    putStrLn "Full Maintenance Test..."
    let aL = toW32Pair [bytes| 01 23 45 67 12 34 56 78 |]
        aR = toW32Pair [bytes| 23 45 67 89 34 56 78 9A |]
        bL = toW32Pair [bytes| 01 23 45 67 12 34 56 78 |]
        bR = toW32Pair [bytes| 23 45 67 89 34 56 78 9A |]
        fs = cycle [f1,f2,f3]
        enc blk keys = coreCrypto 16 keys fs blk

    let go inp@(aL,aR,bL,bR) = force inp `seq` (aL',aR',bL',bR')
         where
            force ((a,b),(c,d),(e,f),(g,h))
                = a `seq` b `seq` c `seq` d
                    `seq` e `seq` f `seq` g `seq` h
            b = subkeys (b0,b1,b2,b3) where ((b0,b1),(b2,b3)) = (bL,bR)
            aL' = enc aL b
            aR' = enc aR b
            a = subkeys (a0,a1,a2,a3) where ((a0,a1),(a2,a3)) = (aL',aR')
            bL' = enc bL a
            bR' = enc bR a
        xs = iterate go (aL,aR,bL,bR)
        (result_a,result_b) = ( fromW32Pair aL `S.append` fromW32Pair aR
                              , fromW32Pair bL `S.append` fromW32Pair bR )
                                where (aL,aR,bL,bR) = xs !! 1000000
                                      (x:xs) !! 0 = x
                                      (x:xs) !! i = force x `seq` xs !! (i-1)
                                      force ((a,b),(c,d),(e,f),(g,h))
                                            = a `seq` b `seq` c `seq` d
                                                `seq` e `seq` f `seq` g `seq` h
        expected_a = [bytes| EE A9 D0 A2 49 FD 3B A6 B3 43 6F B8 9D 6D CA 92 |]
        expected_b = [bytes| B2 C9 5E B0 0C 31 AD 71 80 AC 05 B8 E8 3D 69 6E |]
    if result_a == expected_a && result_b == expected_b
        then putStrLn "passed."
        else putStrLn $ unlines [ "failed."
                                , "expected a = " ++ show (S.unpack expected_a)
                                , "computed a = " ++ show (S.unpack result_a)
                                , "expected b = " ++ show (S.unpack expected_b)
                                , "computed b = " ++ show (S.unpack result_b)  ]
    return ()
